...
def get_ta(hist):
    try:
        ...
        return dict(cur=cur, ma50=ma50, ma200=ma200, hi52=hi52, lo52=lo52,
                    rsi=rsi, macd=macd, above50=cur>ma50, above200=cur>ma200,
...
def chart_earnings(df):
    ...
    try:
        est = df.get('EPS Estimate', pd.Series())
        act = df.get('Reported EPS', pd.Series())
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df.index.astype(str), y=est,
            name='Estimate', marker_color='#334155', opacity=0.8))
        colors = ['#22c55e' if (r >= e) else '#ef4444' for r,e in zip(act, est)]
        fig.add_trace(go.Bar(x=df.index.astype(str), y=act,
...
BULL CASE: 2-3 sentences on the strongest reasons to own this stock for 3+ years.
BEAR CASE: 2-3 sentences on the most credible risks that could derail the thesis.
VERDICT: 2-3 sentences with a clear action: STRONG BUY / BUY / HOLD / TRIM / AVOID. State one specific metric or event that would change your view.
...
def main():
    ...
    <div style="text-align:center;padding:12px 0 6px;">
        <div style="font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:.2em;color:#1e3a5f;text-transform:uppercase;margin-bottom:4px;">◈ PROFESSIONAL STOCK ANALYSIS</div>
        ...
        <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#334155;margin-top:4px;">v30 · yfinance data · 100% free · no API keys required for core analysis</div>
    </div>
    ...
    if not ticker:
        st.markdown("""<div style="text-align:center;padding:70px 20px;color:#1e3a5f;">
            ...
            <div style="font-family:'JetBrains Mono',monospace;font-size:12px;letter-spacing:.1em;">Enter any stock ticker above · NYSE · NASDAQ · Global markets</div>
    ...
    <div class="mm-header">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px;">
        <div>
          ...
          <div style="margin-top:6px;"><span class="chip chip-b">{sector}</span><span class="chip chip-n">{ind}</span></div>
        </div>
    ...
    with sg2:
        ...
        for tab, key in zip(tabs,['12m','24m','36m','60m']):
            with tab:
                ...
                with b1:
                    st.markdown(f'<div class="sc-bear"><div class="sc-label" style="color:#f87171;">🐻 Bear</div><div class="sc-price" style="color:#f87171;">${sc["bear"]["p"]:,.2f}</div><div class="sc-pct" style="color:#ef4444;">{sc["bear"]["pct"]:+.1f}%</div></div>', unsafe_allow_html=True)
                with b2:
                    st.markdown(f'<div class="sc-base"><div class="sc-label" style="color:#93c5fd;">📊 Base</div><div class="sc-price" style="color:#93c5fd;">${sc["base"]["p"]:,.2f}</div><div class="sc-pct" style="color:#60a5fa;">{sc["base"]["pct"]:+.1f}%</div></div>', unsafe_allow_html=True)
                with b3:
                    st.markdown(f'<div class="sc-bull"><div class="sc-label" style="color:#86efac;">🐂 Bull</div><div class="sc-price" style="color:#86efac;">${sc["bull"]["p"]:,.2f}</div><div class="sc-pct" style="color:#22c55e;">{sc["bull"]["pct"]:+.1f}%</div></div>', unsafe_allow_html=True)

    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown('<div class="mm-section-label">🏆 fundamental scorecard</div>', unsafe_allow_html=True)

    sc_cols = st.columns(5)
    for col,(nm,sc_v,pos,neg) in zip(sc_cols,[("Fundamentals",fs,fpos,fneg),("Valuation",vs,vpos,vneg),("Balance Sheet",bs,bpos,bneg),("Momentum",ms,mpos,mneg),("Analyst View",as_,apos,aneg)]):
    ...
    with ac1:
        tgt_mean = safe(info,'targetMeanPrice'); tgt_hi = safe(info,'targetHighPrice')
        tgt_lo = safe(info,'targetLowPrice'); tgt_med = safe(info,'targetMedianPrice')
        ...
        rows_a = (trow("Consensus Rating", rec_key, "#3b82f6") +
                  trow("Rating Score", f"{rec_mean:.2f} / 5.0 (1=Strong Buy)", "#22c55e" if (rec_mean or 3)<2.5 else "#eab308" if (rec_mean or 3)<3.5 else "#ef4444") +
    ...
    with ac2:
        ...
        try:
            if recs is not None and not recs.empty:
                ...
                st.markdown('<div class="alert-b">No recent rating changes available.</div>', unsafe_allow_html=True)
        except:
        ...
        if eh is not None and not eh.empty:
            ...
            for _,row in df8.iterrows():
                ...
                if act is not None and est is not None:
                    ...
                    bc = "#22c55e" if beat else "#ef4444"; bl = "BEAT" if beat else "MISS"
    ...
    with rd1:
        ...
        rows_r = (trow("Beta", fmt(beta_v), "#ef4444" if (beta_v or 1)>1.5 else "#eab308" if (beta_v or 1)>1 else "#22c55e") +
        ...
        try:
            if inst is not None and not inst.empty:
                ...
                for _,row in inst.head(7).iterrows():
                    h_ = str(row.get('Holder','N/A'))[:22]; p_ = row.get('% Out', row.get('pctHeld',None))
    ...
    if has_key:
        ...
        if st.button("⚡ Generate AI Analyst Thesis", type="primary"):
            with st.spinner("Generating professional analysis..."):
                summ = f"Sector:{sector}|Industry:{ind}|Price:${price:.2f}|MarketCap:{fmt_big(mc)}|RevGrowth:{pct_str(rg)}|GrossMargin:{pct_str(safe(info,'grossMargins'))}|NetMargin:{pct_str(safe(info,'profitMargins'))}|ROE:{pct_str(safe(info,'returnOnEquity'))}|ForwardPE:{fmt(safe(info,'forwardPE'),suffix='x')}|PEG:{fmt(safe(info,'pegRatio'))}|Debt/Eq:{fmt(safe(info,'debtToEquity'),suffix='%')}|Beta:{fmt(safe(info,'beta'))}|AnalystTarget:{fmt(safe(info,'targetMeanPrice'),prefix='$')}|Consensus:{rec_key}|RSI:{fmt(ta.get('rsi'))}|Above200MA:{'Yes' if ta.get('above200') else 'No'}|Score:{overall}/10|12m:{ratings['12m']['score']}|60m:{ratings['60m']['score']}"
        ...
        st.markdown('<div class="alert-b" style="font-size:12px;">🤖 <strong>Optional AI Thesis:</strong> Add <code>ANTHROPIC_API_KEY</code> to Streamlit Secrets → Settings → Secrets to unlock AI-generated investment analysis. Core analysis is always 100% free.</div>', unsafe_allow_html=True)

    from datetime import datetime
    st.markdown(f'<div class="footer">Market Mind AI v30 · Data: Yahoo Finance (yfinance) · Generated: {datetime.now().strftime("%Y-%m-%d %H:%M UTC")}<br>⚠️ For informational purposes only · Not financial advice · Always conduct your own research</div>', unsafe_allow_html=True)
